...
def extrair_usernames(caminho_arquivo, chave_lista=None):
    ...
    # Detecta se os dados estão dentro de uma chave
    if isinstance(dados, dict):
        if chave_lista and chave_lista in dados:
...
def obter_dois_arquivos_mais_recentes(pasta):
    ...
    if len(arquivos) >= 2:
        antigo = os.path.join(pasta, arquivos[1])
        novo = os.path.join(pasta, arquivos[0])
        return antigo, novo
    elif len(arquivos) == 1:
        return None, os.path.join(pasta, arquivos[0])
    return None, None
...
def main():
    ...
    if antigo and novo:
        ...
        print(f"\n📄 Comparação feita! Resultado salvo em 'Resultado/lista_resultado.json'")
    else:
        print("\n⚠️ Ainda não há histórico suficiente para comparar (é o primeiro registro salvo).")
